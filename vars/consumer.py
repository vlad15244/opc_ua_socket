...
from .models import Recipe, Trends
from queue import Queue
from .import buffer

plc_buffer = Queue(maxsize=10)
        ...
        if plc_1.Is_Connected:
            for var in var_list.vars:
                if var.is_archive:  # Только архивируемые переменные
                    record = buffer.Buffer(var.ID, float(var.value))
            ...
            if records:
                ...
                plc_buffer.put(records)  # Кладем список записей
                logger.info(
                    f"Сохранено {len(records)} записей в буфер. Размер буфера: {plc_buffer.qsize()}")

                # Для отладки: выводим данные
                for record in records:
                    logger.debug(f"Буфер: {record.to_dict()}")
...
async def flush_buffer_to_db():
    ...
    try:
        ...
        records_to_create = []
        for batch in buffer_items:  # batch — это список BufferRecord
            for record in batch:
        ...
        # Сохраняем в БД (массовое создание)
    ...
    async def periodic_buffer_save(self):
        ...
        while True:
            try:
                ...
                print("буффер")
                await asyncio.sleep(5)  # 60 секунд
